...
import logging 
import yt_dlp 
from tusharsudox import is_authorized, add_sudo, remove_sudo, list_sudo, parse_multiple_durations, auto_cleanup_loop, OWNER_ID, AUTH_CHANNELS, SUDO_USERS
...
def get_greeting_with_emoji():
    ...
    if 0 <= hour < 12:  # 12 AM to 11:59 AM
        return "🌞 𝖦𝗈𝗈𝖽 𝖬𝗈𝗋𝗇𝗂𝗇𝗀"
    elif 12 <= hour < 17:
        return "☀️ 𝖦𝗈𝗈𝖽 𝖠𝖿𝗍𝖾𝗋𝗇𝗈𝗈𝗇"
    elif 17 <= hour < 21:
        return "🌇 𝖦𝗈𝗈𝖽 𝖤𝗏𝖾𝗇𝗂𝗇𝗀"
    else:  # 9 PM to 11:59 PM
        return "🌙 𝖦𝗈𝗈𝖽 𝖭𝗂𝗀𝗁𝗍"
...
async def sudo_handler(bot, m: Message):
    if m.from_user and not is_authorized(m.from_user.id):
        return await m.reply_text("🚫 You are not authorized to use sudo commands.")

    parts = m.text.split()
    if len(parts) < 2:
        return await m.reply_text("⚠ Usage: /sudo add|remove|list ...")

    action = parts[1].lower()

    if action == "add":
        if len(parts) < 4:
            return await m.reply_text("⚠ Usage: /sudo add <id> <time like 2d 3h 5m>")
        try:
            user_id = int(parts[2])
        except:
            return await m.reply_text("❌ Invalid ID")

        duration_text = " ".join(parts[3:])
        duration = parse_multiple_durations(duration_text)
        add_sudo(user_id, duration)
        await m.reply_text(f"✅ Added `{user_id}` to sudo for {duration_text}")

    elif action == "remove":
        if len(parts) < 3:
            return await m.reply_text("⚠ Usage: /sudo remove <id>")
        try:
            user_id = int(parts[2])
        except:
            return await m.reply_text("❌ Invalid ID")
        remove_sudo(user_id)
        await m.reply_text(f"🗑 Removed `{user_id}` from sudo")
...
async def sudo_list_cmd(bot: Client, m: Message):
    if not m.from_user or m.from_user.id != OWNER_ID:
        return await m.reply_text("🚫 Only owner can view sudo list!")
    txt = list_sudo()
...
async def help_command(client: Client, msg: Message):
    help_text = (
        ...
        "`/id` - Get Id🆔\n\n"
        "`/createtxt` - Create .txt file of any text📜\n\n"
...
@bot.on_message(filters.command("createtxt") & filters.private)
async def create_txt_command(_, message: Message):
    await message.reply(
...
async def upload(bot: Client, m: Message):
    ...
    if not (is_authorized(uid) or is_authorized(cid)):
        return await m.reply_text("🚫 You are not authorized to use this bot.")

   # await m.reply_text("✅ Authorized! Send me your TXT file to convert to video...")
