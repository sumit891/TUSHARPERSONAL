...
#also_join = "[▭▬▭▬  Also Join  ▬▭▬▭](https://t.me/+5l-DedecA71hZjFl)"
...
async def start_command(bot: Client, message: Message):
    random_image_url = random.choice(image_urls)  # handler के अंदर random करो
    caption = (
        "**ʜᴇʟʟᴏ👋**\n\n"
    ...
    await bot.send_photo(
        ...
        caption=caption,
        parse_mode="Markdown",  # ये जरूरी है
...
async def create_txt_file(_, message: Message):
    # Command parts
    parts = message.text.split(maxsplit=2)

    if len(parts) < 3:
        await message.reply(
            "❌ Usage: `/createtxt filename.txt your_text_here`",
            quote=True
        )
        return
    ...
    # Send file
    await message.reply_document(filename, caption=f"✅ File `{filename}` created successfully.", quote=True)
