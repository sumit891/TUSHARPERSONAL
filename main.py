...
import logging 
import yt_dlp 
import tushartxthtml
from tusharsudox import add_sudo, sudos, IST, format_remaining, is_authorized, remove_sudo, list_sudo, parse_multiple_durations, auto_cleanup_loop, OWNER_ID, SUDO_USERS, AUTH_CHANNELS
...
async def sudo_add_cmd(bot: Client, message: Message):
    ...
    if len(parts) < 3:
        return await message.reply_text("⚠ Usage: /sudoadd <id> <duration like 2d 3h 5m>")
    ...
    duration_text = " ".join(parts[2:])
    duration = parse_multiple_durations(duration_text)

    now_ts = datetime.now(IST).timestamp()
    existing = await sudos.find_one({"id": user_id})

    if existing:
        if existing.get("permanent"):
            return await message.reply_text(f"⚠ `{user_id}` is already a **Permanent Sudo** ✅")
        
        # Temporary sudo exists → extend
        old_expiry_ts = existing.get("expiry", now_ts)
        if old_expiry_ts < now_ts:
            old_expiry_ts = now_ts  # already expired

        new_expiry_ts = old_expiry_ts + duration.total_seconds()
        await sudos.update_one(
            {"id": user_id},
            {"$set": {"expiry": new_expiry_ts, "permanent": False}}
        )

        old_remaining = format_remaining(old_expiry_ts)
        new_expiry_dt = datetime.fromtimestamp(new_expiry_ts, IST).strftime("%A, %d-%m-%Y %I:%M %p")  # Day included

        msg_text = (
            f"✅ Updated `{user_id}` sudo duration !\n\n"
            f"⏳ Previous remaining : **{old_remaining}**\n"
            f"⏳ New duration added : **{duration_text}**\n"
            f"📅 New expiry : **{new_expiry_dt}**"
        )
    else:
        # Add new sudo
        expiry_ts = now_ts + duration.total_seconds()
        await add_sudo(user_id, duration)
        expiry_dt = datetime.fromtimestamp(expiry_ts, IST).strftime("%A, %d-%m-%Y %I:%M %p")  # Day included

        msg_text = (
            f"✅ Added `{user_id}` to sudo.\n\n"
            f"⏳ Duration : **{duration_text}**\n"
            f"📅 Expiry : **{expiry_dt}**"
        )

    # send message with emoji countdown
    countdown = 60
    emojis = ["✨","🌟","💫","🔥","🎉","🌈","💥","🦋"]
    msg = await message.reply_text(f"{msg_text}\n\n⏳ Auto delete in {countdown} seconds")

    for i in range(countdown-1, 0, -1):
        await asyncio.sleep(1)
        emoji = random.choice(emojis)
        try:
            await msg.edit_text(f"{emoji} {msg_text} {emoji}\n\n⏳ Auto delete in {i} seconds")
        except:
            pass
...
# SUDO REMOVE (Owner only)
@bot.on_message(filters.command("sudoremove"))
async def sudo_remove_cmd(bot: Client, message: Message):
    uid = message.from_user.id if message.from_user else None
    if uid != OWNER_ID:
        return await message.reply_text("🚫 Only Owner can use this command.")
    ...
    parts = message.text.split()
    if len(parts) < 2:
        return await message.reply_text("⚠ Usage: /sudoremove <id>")
    ...
    existing = await sudos.find_one({"id": user_id})
    if not existing:
        return await message.reply_text(f"⚠ `{user_id}` is not in sudo list!")

    await remove_sudo(user_id)

    countdown = 60
    emojis = ["✨","🌟","💫","🔥","🎉","🌈","💥","🦋"]
    msg = await message.reply_text(f"🗑 Removed `{user_id}` from sudo.\n\n⏳ Auto delete in {countdown} seconds")

    for i in range(countdown-1, 0, -1):
        await asyncio.sleep(1)
        emoji = random.choice(emojis)
        try:
            await msg.edit_text(f"{emoji} 🗑 Removed `{user_id}` from sudo {emoji}\n\n⏳ Auto delete in {i} seconds")
        except:
            pass
...
# SUDO LIST (Owner only) 
@bot.on_message(filters.command("sudolist"))
async def sudo_list_cmd(bot: Client, message: Message):
    uid = message.from_user.id if message.from_user else None
    if uid != OWNER_ID:
        await message.reply_text("🚫 Only Owner can view sudo list!")
        return

    chat_id = message.chat.id  # save kar lo pehle

    # User ka /sudolist message turant delete karo
    try:
        await message.delete()
    except:
        pass

    # Ab sudo list show karo
    txt = await list_sudo()
    countdown = 15
    emojis = ["✨", "🌟", "💫", "🔥", "🎉", "🌈", "💥"]
    ...
    # countdown khatam hone ke baad delete
    try:
        await msg.delete()
    except:
        pass
...
async def stop_handler(bot: Client, m: Message):
    ...
    cid = m.chat.id
    if not await is_authorized(user_id=uid, chat_id=cid):
...
async def restart_handler(bot: Client, message: Message):
    uid = message.from_user.id if message.from_user else None
    if uid != OWNER_ID:
    ...
    try:
        ...
        txt = "**✅ Bot Restarted Successfully**"
        countdown = 20
        emojis = ["🦋", "✨", "🌟", "💫", "🔥", "🎉", "🌈"]
        ...
        for i in range(countdown, 0, -1):
            emoji = random.choice(emojis)
            try:
                await msg.edit_text(f"{emoji} {txt} {emoji}\n\n⏳ Auto delete in {i} sec")
            except:
                break
            await asyncio.sleep(1)
...
async def txt_handler(bot: Client, message: Message):
    ...
    editable = await message.reply_text("**Please Send txt file for create html file**")
    input: Message = await bot.listen(editable.chat.id)

    # Delete the "Please Send .txt file" message after user sends file
    await editable.delete()

    # Check if the user sent a .txt file
    if input.document and input.document.file_name.endswith('.txt'):
        # Don't delete user's .txt message
    ...
    # Process content using tushartxthtml.py functions
    urls = tushartxthtml.extract_names_and_urls(file_content)
    videos, pdfs, others = tushartxthtml.categorize_urls(urls)
    html_content = tushartxthtml.generate_html(file_name, videos, pdfs, others)
    ...
    # Send HTML to user
    await message.reply_document(document=html_file_path, caption=f"📚`{b_name}`\n\n💢Open in Chrome.\n\n**👑Created By : Tushar**")
...
async def cookies_handler(client: Client, m: Message):
    uid = m.from_user.id if m.from_user else None
    if uid != OWNER_ID:
        await m.reply_text("🚫 Only Owner can use cookies command.")
        return
    ...
    prompt = await client.send_message(m.chat.id, "**📂 Please upload the cookies file in txt format**")
    ...
    try:
        ...
        if not input_message.document or not input_message.document.file_name.endswith(".txt"):
            return await client.send_message(m.chat.id, "**❌ Invalid file. Please upload a .txt file.**")
        ...
        # countdown msg
        status_msg = await client.send_message(m.chat.id, "**⏳ Validating cookies... (00:00 elapsed)**")
        ...
        await client.send_message(m.chat.id, f"**⚠️ Error while updating cookies: {str(e)}**")
...
# Create txt file of youtube playlist 
...
async def help_command(client: Client, msg):
    help_text = (
        ...
        "`/t2h` - Create html file of .txt file📂\n\n"
        " `/gpd` - Filter pdf url and make txt file from txt file🗃️\n\n"
        "`/cookies` - Upload cookies file(owner)🍪\n\n"
        "`/e2t` - Edit txt file📝\n\n"
...
if __name__ == "__main__":
    bot.loop.create_task(auto_cleanup_loop())  # tusharsudox.py ka loop run karega
    bot.run()
